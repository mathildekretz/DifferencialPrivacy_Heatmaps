...
from User import UserProbabilities, PreProcessing
...
def main():
    ...
    user_proba = PreProcessing('GOWALLA_dataset.txt', l, nb_users=n)
    ...
    plt.savefig('nonprivate_l'+str(l)+'.png')
    plt.clf()
    ...
    for i, eps in enumerate(eps_array) :
        ...
        plt.savefig('private_eps'+str(eps)+'_n'+str(n)+'_l'+str(l)+'.png')    
    ...
    plt.tight_layout()
    plt.savefig('metrics_n'+str(n)+'_l'+str(l)+'.png')
